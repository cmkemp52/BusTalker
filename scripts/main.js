                    ...
                    case "Northbound":
                            mymap.eachLayer(function(tlayer){
                                if(tlayer._icon != undefined){
                                    if(tlayer._popup._content == `<b>Bus Rt: ${jsondata[bus].ROUTE}</b><br>Heading: ${jsondata[bus].DIRECTION}`){
                            ...
                            if(jsondata[bus].ADHERENCE >0){
                                marker.bindPopup(`<b>Bus Rt: ${jsondata[bus].ROUTE}</b><br> Currently ${jsondata[bus].ADHERENCE} minute\(s\) ahead of schedule`);
                            }
                            if(jsondata[bus].ADHERENCE <0){
                                marker.bindPopup(`<b>Bus Rt: ${jsondata[bus].ROUTE}</b><br> Currently ${Math.abs(jsondata[bus].ADHERENCE)} minute\(s\) behind schedule`);
                            }
                            if(jsondata[bus].ADHERENCE ==0){
                                marker.bindPopup(`<b>Bus Rt: ${jsondata[bus].ROUTE}</b><br> Currently on schedule`);
                            }
                            break;
                    case "Southbound":
                            mymap.eachLayer(function(tlayer){
